...
def main():
    # Match the exact same seeding the original main.py does before any
    # data processing, so preprocessing / MVMD decomposition / Elastic Net
    # feature selection produce IDENTICAL results to the original training
    # run. Without this, inference-time RMSE can drift from the recorded
    # training-time RMSE even when using the correct checkpoint, which
    # breaks the checkpoint-matching step below.
